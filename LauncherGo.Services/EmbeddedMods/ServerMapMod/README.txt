...
当前版本（ServerMap 0.4.4 / LauncherGo 2.7.4-pre.3）：
- 模组清单、程序集及地图 API 版本统一为 0.4.4；主程序升级至 2.7.4-pre.3，其他内嵌模组版本不变。
- 修复积雪瓦片楼梯、台阶等被当作独立雪层跳过，地图显示下方方块的问题。积雪变体使用对应 free 变体取色，保留原高度、材质与朝向；无对应变体时保留原方块。
- 更新服务端模组并重启后，旧地表缓存自动在后台重新提取及重绘，期间保留已有地图图片。首次升级有额外读盘开销，无需手动删除缓存或存档；本修复无需更新网页或客户端色表。

此前更新（ServerMap 0.4.3 / LauncherGo 2.7.4-pre.2）：
