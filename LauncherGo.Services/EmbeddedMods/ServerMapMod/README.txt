...
- 本次保持版本 0.3.3，但方向图集通道升级为 v2，服务端和客户端都需要更新本次构建。首次坐骑图像需要同车至少一位骑乘玩家更新客户端 ServerMap；无图时留空，不以图标代替。标准座席/EntityShapeRenderer 接入原版马鹿、船与 VSAirshipmod 主体；单独渲染的特效或附件不保证包含，完整实机外观需上线前验证。详见源代码 docs/server-map-mounts.md。
