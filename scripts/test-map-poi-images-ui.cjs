...
async function main(){const browser=await chromium.launch({headless:true});try{for(const width of [1280,390,320]){
  ...
  await page.route('http://servermap.test/**',async route=>{
    ...
    if(url.pathname.endsWith('/map/metadata'))return json({maxZoom:13,maxZoomOut:12,spawn:{x:0,z:0},tileVersion:'test',colormapReady:true});
  ...
  await page.waitForFunction(()=>Object.values(testMap._layers).some(l=>l.options.title==='Own'));await page.evaluate(()=>Object.values(testMap._layers).find(l=>l.options.title==='Own').openPopup());assert.equal(await page.locator('.poi-image-button').count(),0);
  ...
  if(mobile)await page.locator('#mobileMenu').click();await page.locator('#manageButton').click();await page.locator('#manageModal').waitFor({state:'visible'});assert.equal(await page.locator('#poiImagesEnabledInput').isChecked(),false);await page.locator('[aria-controls="management-uploads"]').click();await page.locator('#poiImagesEnabledInput').check();await page.locator('#manageForm button[type="submit"]').click();await page.locator('#manageModal').waitFor({state:'hidden'});assert.equal(enabled,true);if(mobile)await page.locator('#mobileMenu').click();
  await page.waitForFunction(()=>Object.values(testMap._layers).some(l=>l.options.title==='Own'));await page.evaluate(()=>Object.values(testMap._layers).find(l=>l.options.title==='Own').openPopup());await page.locator('.poi-image-button').waitFor();
  ...
  assert.match(await page.locator('.poi-image-button img').getAttribute('src'),/size=480/);await page.locator('.poi-image-button').click();await page.locator('#poiImageViewer').waitFor({state:'visible'});assert.match(await page.locator('#poiImageFull').getAttribute('src'),/size=0/);await checkIcon(page,'poiImageViewerClose');await page.locator('#poiImageViewerClose').click();assert.equal(await page.locator('#poiImageViewer').isVisible(),false);await page.locator('.poi-image-button').click();await page.keyboard.press('Escape');assert.equal(await page.locator('#poiImageViewer').isVisible(),false);
  await checkThumbnailRatios(page);await page.locator('#languageButton').click();await page.locator('#language').selectOption('en');assert.equal(await page.locator('#poiImageViewerClose').getAttribute('aria-label'),'Close image');assert.equal(await page.locator('#poiImageRemove svg').count(),1);
  ...
  await upload(Buffer.from('GIF89a'));await page.waitForFunction(()=>/allowed list|类型列表/.test(document.getElementById('poiImageError').textContent));assert.equal(saves.length,0);
  await page.evaluate(()=>ServerMapPoiImages.configure({imageTypes:['png','gif','webp']}));
  for(const bytes of [require('./test-map-animated-images.cjs').gif(),require('./test-map-animated-images.cjs').apng()]){await upload(bytes);await page.waitForFunction(()=>!document.getElementById('poiImagePreview').hidden&&!document.getElementById('poiImageError').textContent);}
  await upload(Buffer.alloc(10*1024*1024+1));await page.waitForFunction(()=>document.getElementById('poiImageError').textContent.includes('10 MB'));
  const large=await page.evaluate(()=>{const c=document.createElement('canvas');c.width=8193;c.height=1;return c.toDataURL().split(',')[1];});await upload(Buffer.from(large,'base64'));await page.waitForFunction(()=>document.getElementById('poiImageError').textContent.includes('8192'));
  ...
  await page.waitForFunction(()=>Object.values(testMap._layers).some(l=>l.options.title==='Own'));await page.evaluate(()=>Object.values(testMap._layers).find(l=>l.options.title==='Own').openPopup());await page.locator('.edit-poi').click();await page.locator('#poiImageRemove').click();assert.equal(await page.locator('#poiImagePreviewFrame').isVisible(),false);await page.locator('#poiForm button[type="submit"]').click();await page.locator('#poiModal').waitFor({state:'hidden'});assert.equal(saves[1].imageData,null);
